...
// MARK: - A finished turn leaves on its own

/// "Done" is worth reading, not worth keeping. The tracker stamps the moment a turn
/// finished so a sweep can archive it a fixed time later — and clears the stamp the
/// moment the session does anything else, so a session that picks up again is not
/// swept mid-turn on the strength of an old finish.
@Suite("Finished sessions")
struct FinishedSessionTests {
    @Test func stopStampsTheFinishAndTheNextEventClearsIt() {
        var tracker = SessionTracker()
        tracker.record(id: "s1", kind: "PreToolUse", at: epoch)
        #expect(tracker.sessions["s1"]?.finishedAt == nil)

        tracker.record(id: "s1", kind: "Stop", at: epoch.addingTimeInterval(5))
        #expect(tracker.sessions["s1"]?.finishedAt == epoch.addingTimeInterval(5))

        tracker.record(id: "s1", kind: "PreToolUse", at: epoch.addingTimeInterval(9))
        #expect(tracker.sessions["s1"]?.finishedAt == nil)
    }

    @Test func aFinishedSessionIsListedOnceTheLingerHasPassed() {
        var tracker = SessionTracker()
        tracker.record(id: "s1", kind: "Stop", at: epoch)

        #expect(tracker.finished(olderThan: 20, now: epoch.addingTimeInterval(19)).isEmpty)
        #expect(tracker.finished(olderThan: 20, now: epoch.addingTimeInterval(21)) == ["s1"])
    }

    @Test func aFailedTurnCountsAsFinishedToo() {
        var tracker = SessionTracker()
        tracker.record(id: "s1", kind: "StopFailure", at: epoch)

        #expect(tracker.sessions["s1"]?.status == .failed)
        #expect(tracker.finished(olderThan: 20, now: epoch.addingTimeInterval(30)) == ["s1"])
    }

    @Test func liveBackgroundWorkKeepsASessionOffTheList() {
        var tracker = SessionTracker()
        let agent = BackgroundTask(id: "a1", kind: "subagent", status: "running", label: "yoda")
        tracker.record(id: "s1", kind: "Stop", backgroundTasks: [agent], at: epoch)
        ...
        #expect(tracker.sessions["s1"]?.status == .background)
        #expect(tracker.sessions["s1"]?.finishedAt == nil)
        #expect(tracker.finished(olderThan: 20, now: epoch.addingTimeInterval(3_600)).isEmpty)
    }

    /// A rollout is re-read every poll, and every poll says `idle` again. The finish is
    /// the first of those, not the latest — otherwise the linger restarts forever.
    @Test func repeatedIdleObservationsKeepTheFirstFinish() {
        var tracker = SessionTracker()
        tracker.observe(id: "c1", status: .working, at: epoch)
        tracker.observe(id: "c1", status: .idle, at: epoch.addingTimeInterval(45))
        tracker.observe(id: "c1", status: .idle, at: epoch.addingTimeInterval(50))
        tracker.observe(id: "c1", status: .idle, at: epoch.addingTimeInterval(55))

        #expect(tracker.sessions["c1"]?.finishedAt == epoch.addingTimeInterval(45))
        #expect(tracker.finished(olderThan: 20, now: epoch.addingTimeInterval(66)) == ["c1"])
    }

    @Test func oldestFinishFirst() {
        var tracker = SessionTracker()
        tracker.record(id: "late", kind: "Stop", at: epoch.addingTimeInterval(10))
        tracker.record(id: "early", kind: "Stop", at: epoch)

        #expect(
            tracker.finished(olderThan: 20, now: epoch.addingTimeInterval(60)) == ["early", "late"])
    }
}
