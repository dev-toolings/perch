...
public struct QuietSettings: Codable, Sendable, Equatable {
  ...
  /// Vibe opens the finished turn by default; users can reduce this to a subtle glow.
  ...
  public init(
    duringFocus: Bool = false,
    whenScreenObscured: Bool = false,
    whenScreenShared: Bool = false,
    quietHours: QuietHours? = nil,
    autoExpandOnComplete: Bool = true,
  ...
  public init(from decoder: any Decoder) throws {
    ...
    duringFocus = try container.decodeIfPresent(Bool.self, forKey: .duringFocus) ?? false
    whenScreenObscured =
      try container.decodeIfPresent(Bool.self, forKey: .whenScreenObscured) ?? false
    whenScreenShared =
      try container.decodeIfPresent(Bool.self, forKey: .whenScreenShared) ?? false
    quietHours = try container.decodeIfPresent(QuietHours.self, forKey: .quietHours)
    autoExpandOnComplete =
      try container.decodeIfPresent(Bool.self, forKey: .autoExpandOnComplete) ?? true
